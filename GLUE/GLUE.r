...
CultivarBatchFile<-"IITA-TMS-IBA000070.CSC";
...
WorkDirectory<-getwd();
WorkDirectorySubstrings<-unlist(strsplit(WorkDirectory,"DSSAT48GLUE"));

DSSATD<-paste(WorkDirectorySubstrings[1],"DSSAT48GLUE", sep=""); #Get the working directory of DSSAT.

GLUED<-"/GLUE";#This one is the postion of GLUE under DSSAT and fixed by model developer.
OutputD<-"DSSAT48GLUE"; #This one is the position of DSSAT output and fixed by model developer.
GenotypeD<-"DSSAT48GLUE"; #This one is the position of Genotype file.

WD<-paste(DSSATD, GLUED, sep="");
#OD<-paste(DSSATD, OutputD, sep="");
#GD<-paste(DSSATD, GenotypeD, sep="");

eval(parse(text=paste("ModelRunIndicatorPath='",DSSATD,"/ModelRunIndicator.txt'",sep = '')));
glueWarningLogFile <- file.path(DSSATD, "GlueWarning.txt");
...
eval(parse(text = paste("SimulationControl<-read.csv('",WD,"/SimulationControl.csv', header=T)",sep = '')));
...
eval(parse(text=paste("BatchFile<-readLines('",DSSATD,"/",CultivarBatchFile,"',n=-1)",sep = '')));
...
GenotypeFilePath<-DSSATD;
CurrentGenotypeFile<-paste0(DSSATD, "/",CropName,substr(ModelSelect,3,8),".CUL")
...
BatchFileSetUp(WD, DSSATD, CultivarBatchFile);
...
CulFile.origin= readLines(paste0(DSSATD,"/",GenotypeFileName,".CUL"), encoding="UTF-8")#"/",
...
ModelRun(WD, DSSATD, CropName, GenotypeFileName, CultivarID, RoundOfGLUE, TotalParameterNumber, NumberOfModelRun, RandomMatrix);
...
LikelihoodCalculation(WD, DSSATD, CropName, ParameterNames, RoundOfGLUE);
...
PosteriorDistribution(WD, DSSATD, ParameterNames, ParameterProperty, CropName, RoundOfGLUE);
...
OptimalParameterSet(GLUEFlag, DSSATD, CropName, CultivarID, CultivarName, GenotypeFileName, TotalParameterNumber);
